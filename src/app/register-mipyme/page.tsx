...
export default function RegisterMipymePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) { setSubmitError("El nombre del negocio es obligatorio"); return; }
    ...
    try {
      ...
      const res = await fetch("/api/register-mipyme", {
        method: "POST",
        body: formData,
      ...
      if (res.ok) {
        ...
        setSubmitError(errData.error || "Error al registrar el negocio");
      }
    } catch (err: any) {
      if (err.name === "AbortError") {
        setSubmitError("Tiempo de espera agotado");
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-emerald-50 via-white to-blue-50">
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-emerald-100 shadow-sm">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-emerald-700">Registrar mi negocio</h1>
        </div>
      </header>
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 pb-24">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              >
                <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { if (e.target.files?.[0]) handleLogoFile(e.target.files[0]); }} />
                {logoPreview ? (
                  <div className="flex flex-col items-center gap-2">
                    <img src={logoPreview} alt="Logo" className="w-20 h-20 rounded-xl object-cover border border-gray-200" />
                    <button type="button" onClick={(e) => { e.stopPropagation(); setLogoFile(null); setLogoPreview(null); }} className="text-xs text-red-500 hover:text-red-600 flex items-center gap-1">
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500">Arrastra tu logo o haz clic</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <Input id="name" placeholder="Ej: TeknoCuba" value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-xl" required />
            </div>

            {/* Description */}
            <div>
              <Label htmlFor="description" className="text-sm font-semibold text-gray-800 mb-1.5 block">Descripcion</Label>
              <Textarea id="description" placeholder="Describe tu negocio..." value={description} onChange={(e) => setDescription(e.target.value)} className="rounded-xl min-h-[80px] resize-none" rows={3} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input id="municipality" placeholder="Municipio" value={municipality} onChange={(e) => setMunicipality(e.target.value)} className="h-11 rounded-xl" />
              </div>
            </div>

            {/* Address */}
            <div>
              <Label htmlFor="address" className="text-sm font-semibold text-gray-800 mb-1.5 block">Direccion</Label>
              <Input id="address" placeholder="Ej: Calle 23 #456, Vedado" value={address} onChange={(e) => setAddress(e.target.value)} className="h-11 rounded-xl" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <Label htmlFor="phone" className="text-sm font-semibold text-gray-800 mb-1.5 block">
                  <Phone className="h-4 w-4 inline mr-1.5" /> Telefono
                </Label>
                <Input id="phone" type="tel" placeholder="+53 55512345" value={phone} onChange={(e) => setPhone(e.target.value)} className="h-11 rounded-xl" />
              </div>
              <div>
                <Label htmlFor="whatsapp" className="text-sm font-semibold text-gray-800 mb-1.5 block">
                  <MessageCircle className="h-4 w-4 inline mr-1.5" /> WhatsApp
                </Label>
                <Input id="whatsapp" type="tel" placeholder="+53 55512345" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} className="h-11 rounded-xl" />
              </div>
            </div>
            ...
            <div>
              ...
              <Input id="email" type="email" placeholder="negocio@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 rounded-xl" />
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <Label htmlFor="instagram" className="text-xs text-muted-foreground mb-1 block">
                    <Instagram className="h-3 w-3 inline mr-1" /> Instagram (sin @)
                  </Label>
                  <Input id="instagram" placeholder="mi_negocio" value={instagram} onChange={(e) => setInstagram(e.target.value)} className="h-10 rounded-xl" />
                </div>
                <div>
                  <Label htmlFor="facebook" className="text-xs text-muted-foreground mb-1 block">
                    <Facebook className="h-3 w-3 inline mr-1" /> Facebook
                  </Label>
                  <Input id="facebook" placeholder="MiNegocio" value={facebook} onChange={(e) => setFacebook(e.target.value)} className="h-10 rounded-xl" />
                </div>
              </div>
              <div>
                <Label htmlFor="telegram" className="text-xs text-muted-foreground mb-1 block">
                  <Send className="h-3 w-3 inline mr-1" /> Telegram (sin @)
                </Label>
                <Input id="telegram" placeholder="mi_negocio" value={telegram} onChange={(e) => setTelegram(e.target.value)} className="h-10 rounded-xl" />
              </div>
            </div>

            {/* Services */}
            <div>
              <Label htmlFor="services" className="text-sm font-semibold text-gray-800 mb-1.5 block">Servicios que ofrece</Label>
              <Textarea id="services" placeholder="Ej: Reparacion de celulares, venta de accesorios..." value={services} onChange={(e) => setServices(e.target.value)} className="rounded-xl min-h-[80px] resize-none" rows={3} />
            </div>

            {/* Schedule */}
            <div>
              <Label htmlFor="schedule" className="text-sm font-semibold text-gray-800 mb-1.5 block">
                <Clock className="h-4 w-4 inline mr-1.5" /> Horario
              </Label>
              <Input id="schedule" placeholder="Ej: Lun-Vie 9:00-17:00" value={schedule} onChange={(e) => setSchedule(e.target.value)} className="h-11 rounded-xl" />
            </div>
            ...
            <Button type="submit" disabled={submitting} className="w-full h-12 bg-gradient-to-r from-emerald-500 to-blue-500 hover:from-emerald-600 hover:to-blue-600 text-white text-base font-bold rounded-xl shadow-lg disabled:opacity-60">
              {submitting ? (
                ...
                <><Check className="h-5 w-5 mr-2" /> Registrar negocio</>
              )}
            </Button>
          </form>
        </div>
